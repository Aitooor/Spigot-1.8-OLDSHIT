...
import net.cadiamc.spigot.entity.EntityExecutor;
import net.cadiamc.spigot.entity.MovementCache;
import net.cadiamc.spigot.entity.OptimizedWorldTileEntitySet;
...
public abstract class World implements IBlockAccess {
    ...
    // Spigot end
    protected final List<Entity> g = Lists.newArrayList(); // Paper
    ...
    public final MovementCache movementCache = new MovementCache();
    ...
    public static ExecutorService particleAndSoundExecutor = Executors.newSingleThreadExecutor(new ThreadFactoryBuilder().setNameFormat("PaperSpigot - Particle & Sound Thread").build()); // PaperSpigot - Asynchronous lighting updates
    // Spigot end
        ...
        for (IWorldAccess anU : this.u) {
            if (entity instanceof EntityHuman) {
                if (anU instanceof WorldManager) {
                    ((WorldManager) anU).a((EntityHuman) entity, s, entity.locX, entity.locY, entity.locZ, f, f1);
                } else {
    ...
    public void tickEntities() {
        ...
        if (!this.b.isEmpty()) {
            for (int l = 0; l < this.b.size(); ++l) {
                TileEntity tileentity1 = (TileEntity) this.b.remove(l);
    ...
    private CompletableFuture<List<Entity>> getAsyncEntities(List<Entity> entities) {
        return CompletableFuture.supplyAsync(() -> entities, EntityExecutor.EXECUTOR);
    }
    ...
    public void a(EntityHuman entityhuman, int i, BlockPosition blockposition, int j) {
        try {
            ...
            for (IWorldAccess iWorldAccess : this.u) {
                ((IWorldAccess) iWorldAccess).a(entityhuman, i, blockposition, j);
            }
        } catch (Throwable throwable) {
