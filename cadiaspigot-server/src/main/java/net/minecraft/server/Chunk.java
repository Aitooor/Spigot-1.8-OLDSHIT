...
public class Chunk {
    ...
    private final net.minecraft.server.ChunkSection[] sections;
    ...
    public PacketPlayOutMapChunk.ChunkMap getChunkMap(boolean groundUpContinuous, int primaryBitMask) {
        ...
        for (int i = 0; i < sections.length; ++i) {
            net.minecraft.server.ChunkSection section = sections[i];
            if (section == null) {
    ...
    public Chunk(World world, int i, int j) {
        this.sections = new net.minecraft.server.ChunkSection[16];
        this.e = new byte[256];
        this.f = new int[256];
                ...
                for (int i1 = 0; i1 < short0; ++i1) {
                    ...
                    if (iblockdata.getBlock().getMaterial() != Material.AIR) {
                        ...
                        if (this.sections[k1] == null) {
                            this.sections[k1] = new net.minecraft.server.ChunkSection(k1 << 4, flag);
                        }
    ...
    public net.minecraft.server.ChunkSection[] getSections() {
                ...
                while (true) {
                    ...
                    if (!this.world.worldProvider.o()) {
                        ...
                        do {
                            ...
                            l -= j1;
                            if (l > 0) {
                                net.minecraft.server.ChunkSection chunksection = this.sections[i1 >> 4];
    ...
    private void d(int i, int j, int k) {
        ...
        if (i1 != l) {
            ...
            if (!this.world.worldProvider.o()) {
                net.minecraft.server.ChunkSection chunksection;
                ...
                while (i1 > 0 && l1 > 0) {
                    ...
                    net.minecraft.server.ChunkSection chunksection1 = this.sections[i1 >> 4];
    ...
    private Block getType(int i, int j, int k) {
        ...
        if (j >= 0 && j >> 4 < this.sections.length) {
            net.minecraft.server.ChunkSection chunksection = this.sections[j >> 4];
    ...
    public IBlockData getBlockData(final BlockPosition blockposition) {
        if (blockposition.getY() >= 0 && blockposition.getY() >> 4 < this.sections.length) {
            net.minecraft.server.ChunkSection chunksection = this.sections[blockposition.getY() >> 4];
            if (chunksection != null) {
    ...
    public IBlockData a(BlockPosition blockposition, IBlockData iblockdata) {
        ...
        if (iblockdata1 == iblockdata) {
            ...
            if (chunksection == null) {
                ...
                chunksection = this.sections[j >> 4] = new net.minecraft.server.ChunkSection(j >> 4 << 4, !this.world.worldProvider.o());
                flag = j >= i1;
            }
    ...
    public void a(EnumSkyBlock enumskyblock, BlockPosition blockposition, int i) {
        ...
        net.minecraft.server.ChunkSection chunksection = this.sections[k >> 4];

        if (chunksection == null) {
            chunksection = this.sections[k >> 4] = new net.minecraft.server.ChunkSection(k >> 4 << 4, !this.world.worldProvider.o());
            this.initLighting();
        }
    ...
    public net.minecraft.server.ChunkCoordIntPair j() {
        return new net.minecraft.server.ChunkCoordIntPair(this.locX, this.locZ);
    }
    ...
    public void a(net.minecraft.server.ChunkSection[] achunksection) {
    ...
    public Chunk clone() {
        Chunk chunk = new Chunk(world, locX, locZ);
        System.arraycopy(e, 0, chunk.e, 0, chunk.e.length);
        System.arraycopy(f, 0, chunk.f, 0, chunk.f.length);
        System.arraycopy(g, 0, chunk.g, 0, chunk.g.length);
        chunk.h = h;
        System.arraycopy(heightMap, 0, chunk.heightMap, 0, chunk.heightMap.length);
        chunk.k = k;
        chunk.tileEntities.putAll(tileEntities);
        chunk.done = done;
        chunk.lit = lit;
        chunk.p = p;
        chunk.q = q;
        ...
        chunk.v = v;
        chunk.w = w;
        chunk.emptySectionBits = emptySectionBits;
        chunk.neighbors = neighbors;
        chunk.pendingLightUpdates = new AtomicInteger(pendingLightUpdates.get());

        for (int i = 0; i < sections.length; i++) {
            net.minecraft.server.ChunkSection section = sections[i];
            if (section != null) {
                net.minecraft.server.ChunkSection s = new net.minecraft.server.ChunkSection(section.getYPosition(), false, section.getIdArray().clone());
                s.a(section.getEmittedLightArray());
                s.b(section.getSkyLightArray());
                chunk.sections[i] = s;
            }
        }
        return chunk;
