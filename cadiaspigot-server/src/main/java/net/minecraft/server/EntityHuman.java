...
public abstract class EntityHuman extends EntityLiving {
        ...
        if (entity.aD()) {
            if (!entity.l(this)) {
                ...
                if (f > 0.0F || f1 > 0.0F) {
                    ...
                    KnockbackProfile knockbackProfile = (getKbProfile() == null ? SpigotConfig.globalKbProfile : getKbProfile());
                    if (damaged) {
                        // kohi start
                        // uDrop KB Start
                        if (i > 0) {
                            entity.g(
                                    (-MathHelper.sin(this.yaw * 3.1415927f / 180.0f) * (float) i * knockbackProfile.getExtraHorizontal()), // extra Horizontal
                                    knockbackProfile.getExtraVertical(),  // extra Vertical
                                    (MathHelper.cos(this.yaw * 3.1415927f / 180.0f) * (float) i * knockbackProfile.getExtraHorizontal())); // extra Horizontal
    ...
    static class SyntheticClass_1 {
        ...
        static {
            try {
                SyntheticClass_1.a[EnumDirection.SOUTH.ordinal()] = 1;
            } catch (NoSuchFieldError nosuchfielderror) {
                ;
            }

            try {
                SyntheticClass_1.a[EnumDirection.NORTH.ordinal()] = 2;
