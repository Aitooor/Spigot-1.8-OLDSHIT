...
import online.nasgar.spigot.util.CraftPotionUtils;
...
public abstract class EntityLiving extends Entity {
    ...
    public final Map<Integer, MobEffect> effects = Maps.newConcurrentMap();
        ...
        while (iterator.hasNext()) {
            ...
            if (!mobeffect.tick(this)) {
                if (!this.world.isClientSide) {
                    PotionEffectExpireEvent event = new PotionEffectExpireEvent((LivingEntity) this.getBukkitEntity(), CraftPotionUtils.toBukkit(mobeffect));
                    this.world.getServer().getPluginManager().callEvent(event);
                    PotionEffectExpireEvent event2 = new PotionEffectExpireEvent((LivingEntity) this.getBukkitEntity(), CraftPotionUtils.toBukkit(mobeffect));
    ...
    public void addEffect(MobEffect mobeffect) {
        this.addEffect(mobeffect, PotionEffectAddEvent.EffectAddReason.UNKNOWN);
    }

    public void addEffect(MobEffect mobeffect, PotionEffectAddEvent.EffectAddReason cause) {
        // CraftBukkit start
        if (isTickingEffects) {
        ...
        if (this.d(mobeffect)) {
            ...
            if (current != null) {
                PotionEffectExtendEvent event = new PotionEffectExtendEvent((LivingEntity) this.getBukkitEntity(), CraftPotionUtils.toBukkit(mobeffect), CraftPotionUtils.toBukkit(current), cause);
                this.world.getServer().getPluginManager().callEvent(event);
                ...
                PotionEffectAddEvent event = new PotionEffectAddEvent((LivingEntity) this.getBukkitEntity(), CraftPotionUtils.toBukkit(mobeffect), cause);
                this.world.getServer().getPluginManager().callEvent(event);
                if (event.isCancelled()) {
                    return;
                }
                PotionEffectAddEvent event2 = new PotionEffectAddEvent((LivingEntity) this.getBukkitEntity(), CraftPotionUtils.toBukkit(mobeffect), PotionEffectAddEvent.EffectAddReason.values()[cause.ordinal()]);
                this.world.getServer().getPluginManager().callEvent(event2);
                if (event2.isCancelled()) {
                    return;
                }
    ...
    public void removeEffect(int i) {
        ...
        if (mobeffect != null) {
            PotionEffectRemoveEvent event = new PotionEffectRemoveEvent((LivingEntity) this.getBukkitEntity(), CraftPotionUtils.toBukkit(mobeffect));
            this.world.getServer().getPluginManager().callEvent(event);
        ...
        if (this.isInvulnerable(damagesource)) {
            ...
            if (this.getHealth() <= 0.0F) {
                ...
                if ((float) this.noDamageTicks > (float) this.maxNoDamageTicks / 2.0F) {
                    ...
                    this.activatedTick = MinecraftServer.currentTick + (this.maxNoDamageTicks + 1);
                }
    ...
    public void a(Entity entity, float f, double d0, double d1) {
        if (this.random.nextDouble() >= this.getAttributeInstance(GenericAttributes.c).getValue()) {
            ...
            float f3 = profile.getHorizontal();

            this.motX /= profile.getFriction(); // Friction
            this.motY /= profile.getFriction(); // Friction
            this.motZ /= profile.getFriction(); // Friction

            this.motX -= d0 / magnitude * f3; // Horizontal
            this.motY += profile.getVertical(); // Vertical
            this.motZ -= d1 / magnitude * f3; // Horizontal

            if (this.motY > profile.getVerticalLimit()) {
                this.motY = profile.getVerticalLimit(); // Vertical Limit
            }
        }
    }
