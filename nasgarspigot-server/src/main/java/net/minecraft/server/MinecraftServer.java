...
public abstract class MinecraftServer implements Runnable, ICommandListener, IAsyncTaskHandler, IMojangStatistics {
    ...
    private ConcurrentLinkedQueue<FutureTask<?>> processQueue = new ConcurrentLinkedQueue<>();
    ...
    private long connectionCurTime = System.nanoTime(), curTime = System.nanoTime(), tickSection = curTime;
    ...
    public void run() {
        try {
            if (this.init()) {
                ...
                executor.scheduleAtFixedRate(() -> {
                    FutureTask<?> entry;
                    int count = this.processQueue.size();
                    while (count-- > 0 && (entry = this.processQueue.poll()) != null) {
...
//            entry.run();
    ...
    public <V> ListenableFuture<V> a(Callable<V> callable) {
        ...
        if (!this.isMainThread()) { // CraftBukkit && !this.isStopped()) {
            ...
            // Spigot start
            processQueue.add(listenablefuturetask);
